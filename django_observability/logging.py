...
class StructuredLogger:
    ...
    def _should_log_body(self, request: HttpRequest) -> bool:
        ...
        return content_type.startswith(('application/json', 'text/', 'multipart/form-data'))
    ...
    def log_request_start(self, request: HttpRequest, correlation_id: str) -> None:
        ...
        try:
            ...
            if self.config.get('LOGGING_INCLUDE_BODY', False) and self._should_log_body(request):
                try:
            ...
            self.logger.error("Failed to log request start", exc_info=True, extra={"correlation_id": correlation_id})
    
    def log_request_end(self, request: HttpRequest, response: HttpResponse, duration: float, correlation_id: str) -> None:
        """
        ...
        try:
            ...
            if self.config.get('LOGGING_INCLUDE_BODY', False):
                ...
                if content_type.startswith(('application/json', 'text/')):
                    try:
                        log_data["http"]["response_body"] = response.content.decode('utf-8')
            ...
            self.logger.error("Failed to log request end", exc_info=True, extra={"correlation_id": correlation_id})
    
    def log_exception(self, request: HttpRequest, exception: Exception, correlation_id: str) -> None:
        """
        ...
        try:
            ...
            self.logger.error("Failed to log exception", exc_info=True, extra={"correlation_id": correlation_id})
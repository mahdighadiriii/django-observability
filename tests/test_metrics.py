    ...
    with patch('django.db.connection.cursor', return_value=mock_cursor):
        with patch('django_observability.metrics.logger') as mock_logger:
            ...
            try:
                ...
                pass
            assert any("Query failed" in str(call) for call in mock_logger.error.call_args_list)
...
def test_instrument_cache_error(config):
    ...
    with patch('django.core.cache.caches', return_value={'default': mock_cache}):
        with patch('django_observability.metrics.logger') as mock_logger:
            ...
            assert any("Failed to record cache operation" in str(call) for call in mock_logger.error.call_args_list)
...
def test_record_cache_operation(config):
    ...
    collector = MetricsCollector(config)
    collector.record_cache_operation(cache_name="default", operation="get", result="hit")
    metrics = collector.get_metrics()
...
def test_get_request_size(request_factory, config):
    ...
    collector = MetricsCollector(config)
    request = request_factory.post('/test/', data={'key': 'value'}, content_type='application/x-www-form-urlencoded')
        ...
        with patch('django_observability.metrics.logger') as mock_logger:
            with patch('time.time', side_effect=[1000.0, 1000.1]):
                ...
                try:
                    ...
                    debug_calls = [str(call) for call in mock_logger.debug.call_args_list]
                    assert any("Cache get_many completed" in call for call in debug_calls)
                    assert 'test_app_django_cache_operations_total' in collector.get_metrics()
                finally:
...
def test_custom_histogram_error(config):
    ...
    with patch('prometheus_client.Histogram', side_effect=Exception("Histogram error")):
        with patch('django_observability.metrics.logger') as mock_logger:
            result = collector.create_custom_histogram("test_histogram", "Test histogram", ["label"])
            assert result is None
...
def test_custom_counter_error(config):
    ...
    with patch('prometheus_client.Counter', side_effect=Exception("Counter error")):
        with patch('django_observability.metrics.logger') as mock_logger:
            result = collector.create_custom_counter("test_counter", "Test counter", ["label"])
            assert result is None
            mock_logger.error.assert_called_once_with(
...
def test_custom_gauge_error(config):
    ...
    with patch('prometheus_client.Gauge', side_effect=Exception("Gauge error")):
        with patch('django_observability.metrics.logger') as mock_logger:
            result = collector.create_custom_gauge("test_gauge", "Test gauge", ["label"])
            assert result is None
            mock_logger.error.assert_called_once_with(
...
def test_record_db_query_error(config):
    ...
    collector = MetricsCollector(config)
    with patch.object(collector, 'django_db_queries_total', side_effect=Exception("Metric error")):
        with patch('django_observability.metrics.logger') as mock_logger:
            collector.record_db_query(db_alias="default", query_type="SELECT", duration=0.05)
            assert any("Failed to record DB query metrics" in str(call) for call in mock_logger.error.call_args_list)
...
def test_record_request_duration_error(config, request_factory):
    ...
    with patch.object(collector, '_get_endpoint_label', side_effect=Exception("Label error")):
        with patch('django_observability.metrics.logger') as mock_logger:
            collector.record_request_duration(request, response, 0.1)
            assert any("Failed to record request duration" in str(call) for call in mock_logger.error.call_args_list)

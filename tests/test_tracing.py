...
def test_tracing_otlp_failure(config, mocker):
    ...
    mocker.patch('django_observability.tracing.OTLPSpanExporter', side_effect=Exception("Invalid endpoint"))
    ...
    assert isinstance(span_exporter, ConsoleSpanExporter), f"Expected ConsoleSpanExporter, got {type(span_exporter)}"

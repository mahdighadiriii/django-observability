...
def test_django_integration_init_with_pytest_env():
    ...
    with patch.dict(os.environ, {"PYTEST_CURRENT_TEST": "test", "DJANGO_ENABLE_TEST_INTEGRATION": "False"}):
    ...
    with patch.dict(os.environ, {"PYTEST_CURRENT_TEST": "test", "DJANGO_ENABLE_TEST_INTEGRATION": "True"}):
        with patch.object(config, 'get', return_value=True) as mock_config_get:
            with patch('opentelemetry.trace.get_tracer') as mock_tracer:
                with patch('opentelemetry.instrumentation.redis.RedisInstrumentor') as mock_redis:
                    with patch('opentelemetry.instrumentation.psycopg2.Psycopg2Instrumentor') as mock_psycopg:
                        with patch('opentelemetry.instrumentation.dbapi.DatabaseApiIntegration') as mock_dbapi:
                            integration = django_integration.DjangoIntegration(config)
                            mock_tracer.assert_called_once_with('django_observability.django_integration')
                            assert integration.config == config
                            assert any(
                                call.args == ('ENABLE_TEST_INTEGRATION', False)
                                for call in mock_config_get.call_args_list
...
def test_setup_integrations():
    ...
    with patch.dict(os.environ, {
        ...
        with patch.object(config, 'get', side_effect=lambda key, default: True if key in ['INTEGRATE_DB_TRACING', 'INTEGRATE_CACHE_TRACING', 'INTEGRATE_TEMPLATE_TRACING'] else default):
            integration = django_integration.DjangoIntegration(config)
    ...
    with patch.dict(os.environ, {
        ...
        with patch.object(config, 'get', side_effect=lambda key, default: False if key in ['INTEGRATE_DB_TRACING', 'INTEGRATE_CACHE_TRACING', 'INTEGRATE_TEMPLATE_TRACING'] else default):
            integration = django_integration.DjangoIntegration(config)
            integration._setup_integrations()
...
def test_instrument_database_failure():
    ...
    with patch('opentelemetry.instrumentation.psycopg2.Psycopg2Instrumentor', side_effect=ImportError):
        with patch('django_observability.django_integration.logger') as mock_logger:
            integration._instrument_database()
            mock_logger.warning.assert_called_once_with("Psycopg2 instrumentation not available")
    ...
    with patch('opentelemetry.instrumentation.psycopg2.Psycopg2Instrumentor') as mock_psycopg:
        with patch('opentelemetry.instrumentation.dbapi.DatabaseApiIntegration') as mock_dbapi:
            ...
            with patch('django_observability.django_integration.logger') as mock_logger:
                integration._instrument_database()
                mock_logger.error.assert_called_with("Failed to instrument psycopg2: DB error", exc_info=True)
...
def test_instrument_cache():
    ...
    with patch('opentelemetry.instrumentation.redis.RedisInstrumentor', side_effect=ImportError):
        with patch('django_observability.django_integration.logger') as mock_logger:
            integration._instrument_cache()
            mock_logger.warning.assert_called_with("Redis instrumentation not available")
...
def test_instrument_templates():
    ...
    with patch('django_observability.django_integration.engines') as mock_engines:
        ...
        with patch('django_observability.django_integration.logger') as mock_logger:
            integration._instrument_templates()
            mock_logger.error.assert_called_with("Failed to instrument templates: Template error", exc_info=True)
...
def test_wrap_template_engine():
    ...
    config = get_config()
    config.force_reload()
    with patch.dict(os.environ, {"PYTEST_CURRENT_TEST": "", "DJANGO_ENABLE_TEST_INTEGRATION": "True"}):
